"""
Regenerate Overpot_dur_Air_15bp_Main/Other_300mA.png for the 260603 folder,
using 260603-specific VC Polyol data (reproducibility MEA).
All other samples fall back to the main ALT_DATA path.
"""
...
NEW_DATA = os.path.join(BASE, "260603 New data set (reproducibility)", "Overpotnital")
...
def find_file(cond_folder, dur_folder, stem, samp):
    # For VC Polyol Air BP, use 260603 reproducibility data
    if samp == "VC Polyol" and cond_folder == "Air BP":
        for dur_try in (dur_folder, dur_folder.lower()):
            folder = os.path.join(NEW_DATA, cond_folder, dur_try)
            if os.path.isdir(folder):
                for fn in sorted(os.listdir(folder)):
                    if fn.endswith(".xlsx") and fn.lower().startswith(stem.lower()):
                        fp = os.path.join(folder, fn)
                        try:
                            wb = openpyxl.load_workbook(fp, read_only=True)
                            ok = "Sheet1" in wb.sheetnames
                            wb.close()
                            if ok:
                                return fp
                        except Exception:
...
def build(sample_list, group_name, legend_frameon):
    cond_folder = "Air BP"
    nrows     = len(DURS)
    ...
    for row_idx, (dur_label, dur_folder) in enumerate(DURS):
        for col_idx, samp in enumerate(sample_list):
            ...
            fp   = find_file(cond_folder, dur_folder, stem, samp) if stem else None
            ...
            if fp is None:
                ...
                try:
                    ...
                    print(f"  {samp}/{dur_label}: {os.path.basename(fp)}")
                except Exception as e:
                    print(f"  ERROR {samp}/{dur_label}: {e}")
    ...
    out = os.path.join(OUT_DIR, f"Overpot_dur_Air_15bp_{group_name}_300mA.png")
    fig.savefig(out, dpi=300, bbox_inches="tight")
    plt.close(fig)
    print(f"Saved: {out}")


build(MAIN_SAMPLES,  "Main",  legend_frameon=True)
build(OTHER_SAMPLES, "Other", legend_frameon=False)
print("Done.")
