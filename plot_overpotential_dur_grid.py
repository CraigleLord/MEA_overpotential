"""
Durability overpotential breakdown grids.
One figure per condition (4 conditions × 2 groups = 8 PNGs).
Layout: rows = BOL / 30K / 75K,  columns = samples (4).
Legend only in top-left panel.
Output: All figures important/Overpot_dur_{cond}_{group}.png
"""
...
# ---------------------------------------------------------------------------
# Conditions — (cond_folder, safe_name, display_label, is_15bp)
# ---------------------------------------------------------------------------
CONDITIONS = [
...
def build_dur_grid(sample_list, cond_folder, safe_name, is_15bp, group_name):
    ...
    for row_idx, (dur_label, dur_folder) in enumerate(DURS):
        for col_idx, samp in enumerate(sample_list):
            ...
            if row_idx == 0:
                ax.set_title(COL_LABELS[samp], fontsize=fs + 2,
                             fontweight="bold", pad=4)

            # Y-axis label: durability label (bold) for left column, plain otherwise
            if col_idx == 0:
...
# Run
# ---------------------------------------------------------------------------
for cond_folder, safe_name, cond_label, is_15bp in CONDITIONS:
