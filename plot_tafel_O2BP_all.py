...
OUT   = os.path.join(BASE, "Durability I-V figure", "Tafel_O2BP")
...
# BOL/30K/75K: marker color, regression line color, line style, folder
DURS = [
...
for samp_lbl, fname in SAMPLES:
    ...
    for dur_lbl, dur_dir, mcolor, lcolor, lst, mkr in DURS:
        j, V = load_iv("O2 BP", dur_dir, fname)
        if j is None:
        ...
        if slope is not None:
            ...
            n_pts = int(((np.log(j) >= LN_LO) & (np.log(j) <= LN_HI)).sum())
            ann_lines.append(
    ...
    out  = os.path.join(OUT, f"Tafel_{safe}_O2BP.png")
